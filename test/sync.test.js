...
beforeEach(() => {
  ...
  cfg = {
    origins: [{ owner: "temporalio", repo: "samples-typescript" }],
...
test("Cleans snippets from files that were not cleaned up previously", async () => {
  fs.copyFileSync(
    `${fixturesPath}/index_with_code.md`,
    `${tutorialsPath}/index_with_code.md`
  );
...
test.skip("Per snippet selectedLines configuration", async () => {
  (cfg.origins = [
...
test.skip("Per snippet highlightedLines configuration", async () => {
  (cfg.origins = [
    { owner: "temporalio", repo: "money-transfer-project-template-go" },
  ]),
    fs.copyFileSync(
      `${fixturesPath}/highlight.md`,
      `${tutorialsPath}/highlight.md`
    );
  ...
  const expected = fs.readFileSync(
    `test/fixtures/expected-highlight.md`,
    "utf8"
  );
  expect(data).toMatch(expected);
});
